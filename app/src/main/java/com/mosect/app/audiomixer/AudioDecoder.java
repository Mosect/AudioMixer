...
public class AudioDecoder {
    ...
    private boolean syncPlayTime = true;

    public AudioDecoder(Context context, String assetsName) {
        this.context = context;
        this.assetsName = assetsName;
    }

    public void setSyncPlayTime(boolean syncPlayTime) {
        this.syncPlayTime = syncPlayTime;
    }

    public boolean isSyncPlayTime() {
        return syncPlayTime;
    }
    ...
    private void loop() {
        ...
        try (AssetFileDescriptor afd = context.getAssets().openFd(assetsName)) {
            ...
            configureFormat(mediaFormat, sampleRate, channelCount);
                    ...
                    if (index >= 0) {
                        ...
                        if (!config && bufferInfo.size > 0) {
                            ...
                            if (lastFrameTime >= 0) {
                                ...
                                if (syncPlayTime && framePartTime > systemPartTime && systemPartTime >= 0) {
    ...
    private void configureFormat(MediaFormat format, int sampleRate, int channelCount) {
        Callback callback = this.callback;
        if (null != callback) {
            callback.onFormatConfigured(format, sampleRate, channelCount);
        }
    }
    ...
    public interface Callback {
        ...
        void onFormatConfigured(MediaFormat format, int sampleRate, int channelCount);

        void onWritePcm(ByteBuffer data);
    }
}
